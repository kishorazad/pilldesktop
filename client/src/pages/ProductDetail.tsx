...
// ✅ Optional: define type (prevents TS errors)
interface Product {
  id: number;
  name: string;
  price: number;
...
const ProductDetail = () => {
  const params = useParams<{ id: string }>(); // ✅ typed param
  ...
  const {
    ...
    queryFn: () => api.getProductById(params.id),
    enabled: !!params.id,
  });
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <img
        src={product.imageUrl || '/pillnow.png'}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {relatedProducts.map((item: Product) => (
          <Link key={item.id} href={`/products/${item.id}`}>
            <div className="border p-2 rounded cursor-pointer hover:shadow">
              <img
                src={item.imageUrl || '/pillnow.png'}
